...
function UpdateForm({ data }) {
  ...
  const handleOnChangeNoKtp = (value) => {
    ...
    setError({
      ...error,
      noKtp: (value =
        value.length < 14 || value.length > 14
          ? "Format KTP harus berupa 14 karakter angka"
          : ""),
    });
  };
  ...
  const handleOnChangeBirthdateMonth = (value) => {
    ...
    setMustahik({
      ...mustahik,
      birthdate:
        mustahik.birthdate.slice(0, 5) + value + mustahik.birthdate.slice(7),
    });
  };
    ...
    <form onSubmit={(e) => handleOnSubmit(e)}>
      ...
      <div className="form" id="foto-mustahik">
        <FileField
          ...
          description={
            "Unggah foto ukuran 300 x 300 milik mustahik dengan format .jpg"
          }
...
function UpdatePage() {
  ...
  if (error) {
    console.error(error);
    return [error].map(({ message }, id) => <p key={id}>{message}</p>);
  }
